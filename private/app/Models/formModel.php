...
{
    public function getTableau1Results($plon_date)
    {
        $results = DB::table('plongee AS plo')
            ->join('inscrire AS ins', function ($join) {
                $join->on('ins.plon_date', '=', 'plo.plon_date')
                    ->on('ins.sea_id', '=', 'plo.sea_id');
            })
            ->join('adherent AS adh', 'adh.ad_email', '=', 'plo.plon_directeur')
            ->join('lieu AS lie', 'lie.lieu_id', '=', 'plo.lieu_id')
            ->select('plo.plon_date', 'adh.ad_nom', 'lieu_nom', DB::raw('COUNT(adh.ad_email) AS count'))
            ->where('plo.plon_date', $plon_date)
            ->groupBy('plo.plon_date', 'adh.ad_nom', 'lieu_nom')
            ->get();

        return $results;
    }

    public function getTableau2Results($plon_date)
    {
        $results = DB::table('plongee AS plo')
            ->join('adherent AS adh', 'adh.ad_email', '=', 'plo.PLON_SECURITE')
            ->select('adh.ad_nom')
            ->where('plo.plon_date', $plon_date)
            ->get();
    ...
    public function getTableau3Results($sea_id, $plon_date)
    {
        $results = DB::select("SELECT count(*) as nb FROM PALANQUE WHERE SEA_ID = ? AND PLON_DATE = ?", [$sea_id, $plon_date]);
        return $results[0]->nb;
    }
}
